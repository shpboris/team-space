...
public class EnterpriseDeployManagerImpl implements EnterpriseDeployManager {
    ...
    public static final String NW_RDS_INSTANCE_CF_TEMPLATE_CLASSPATH_LOCATION = "classpath:aws-network-rds-instances.template";
    ...
    public DeployResponse createEnvironment(DeployEnterpriseModeRequest deployEnterpriseModeRequest) {
        ...
        String sitePublicDns = null;

        if(deployEnterpriseModeRequest.isSingleStackDeployment()){
            Stack networkRdsInstancesStack = createNetworkRdsInstancesStack(deployEnterpriseModeRequest);
            sitePublicDns = stackManagerService.getStackOutput(networkRdsInstancesStack, STACK_OUTPUT_SITE_PUBLIC_DNS_KEY);
        } else {
            ...
            sitePublicDns = stackManagerService.getStackOutput(instancesStack, STACK_OUTPUT_SITE_PUBLIC_DNS_KEY);
        }
    ...
    public void destroyEnvironment(UndeployEnterpriseModeRequest undeployEnterpriseModeRequest) {
        if(undeployEnterpriseModeRequest.isSingleStackDeployment()){
            destroyNetworkRdsInstancesStack(undeployEnterpriseModeRequest);
        } else {
    ...
    private Stack createNetworkRdsInstancesStack(DeployEnterpriseModeRequest deployEnterpriseModeRequest){
        String networkRdsInstancesStackName = getStackFullName(deployEnterpriseModeRequest.getEnvTag(),
                NETWORK_RDS_INSTANCES_STACK_NAME);
        List<Parameter> networkRdsInstancesStackParameters = getNetworkRdsInstancesStackParameters(deployEnterpriseModeRequest);
        stackManagerService.createStack(networkRdsInstancesStackName,
                NW_RDS_INSTANCE_CF_TEMPLATE_CLASSPATH_LOCATION, networkRdsInstancesStackParameters);
        Stack networkRdsInstancesStack = stackManagerService.waitForStackCreation(networkRdsInstancesStackName,
                ENTERPRISE_MODE_FULL_STACK_CF_MAX_RETRIES);
        return networkRdsInstancesStack;
    }
    ...
    private void destroyNetworkRdsInstancesStack(UndeployEnterpriseModeRequest undeployEnterpriseModeRequest) {
        String networkRdsInstancesStackName = getStackFullName(undeployEnterpriseModeRequest.getEnvTag(),
                NETWORK_RDS_INSTANCES_STACK_NAME);
        stackManagerService.deleteStack(networkRdsInstancesStackName);
        stackManagerService.waitForStackDeletion(networkRdsInstancesStackName, ENTERPRISE_MODE_FULL_STACK_CF_MAX_RETRIES);
    }
    ...
    private List<Parameter> getNetworkRdsInstancesStackParameters(DeployEnterpriseModeRequest deployEnterpriseModeRequest){
        ...
        String userDataStr = userDataHelper.getUserDataScript(INITIAL_USER_DATA_CLASSPATH_LOCATION, deployEnterpriseModeRequest.getArtifactName(),
                deployEnterpriseModeRequest.getRegion(), bucketName, DB_MODE_RDS, "",
        ...
        return Arrays.asList(envTag, instanceType, instanceCount, userData, dbName, dbUsername, dbPassword);
    }
    ...
    private List<Parameter> getInstancesStackParameters(DeployEnterpriseModeRequest deployEnterpriseModeRequest,
        ...
        String bucketName = AwsEntitiesHelperUtil
                .getEntityName(deployEnterpriseModeRequest.getEnvTag(), BUCKET_ENTITY_TYPE).toLowerCase();
        String userDataStr = userDataHelper.getUserDataScript(USER_DATA_CLASSPATH_LOCATION, deployEnterpriseModeRequest.getArtifactName(),
